    ...
    <div class="card">
      <div class="card-body" style="text-align:center;padding:28px 22px">
        ...
        <div id="photoWrap" style="position:relative;display:inline-block;margin-bottom:16px;cursor:pointer"
          ...
          <?php else: ?>
          <div id="photoPreview" class="profile-photo" style="width:88px;height:88px;font-size:2.2rem;
               margin:0;cursor:pointer"><?=$init?></div>
          <?php endif; ?>
          <!-- camera overlay -->
        ...
        <div style="margin-top:12px">
          <span class="badge badge-<?=$user['is_approved']==='approved'?'success':'warning'?>">
            <?=ucfirst($user['is_approved'])?>
          </span>
        </div>
        <div style="font-size:.72rem;color:var(--tx3);margin-top:10px">
      ...
      <div class="card-body">
        <form method="POST" action="<?=BASE_URL?>/application/profile.php" enctype="multipart/form-data" id="photoForm">
          ...
          <div id="photoDropzone"
            ...
            <div style="font-size:.8rem;color:var(--tx3);font-weight:600" id="dropText">
  ...
  <div class="card">
    ...
    <div class="card-body">
      <form method="POST" action="<?=BASE_URL?>/application/profile.php">
        ...
        <div class="form-group">
          <label>Class / Year / Section</label>
          <input type="text" class="form-control" name="class_name" value="<?=htmlspecialchars($student['class_name'] ?? '')?>">
        </div>
